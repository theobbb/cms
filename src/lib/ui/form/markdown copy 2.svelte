...
<div class="relative w-full bg-surface text-surface-foreground">
	<Label {label} {id} {required} icon="icon-[ri--text-block]" />

	<div class="absolute top-0 right-1.5 flex h-8 items-center">
		<Button variant="ghost" size="sm" icon="icon-[ri--information-line]" />
	</div>
	<header class="flex- items-center justify-between gap-4 border border-b-0 py-1.5 pr-1.5 pl-2.5">
		<div class="flex justify-between gap-gap border-b">
			<div class="flex w-fit items-center gap-1">
				{#each views as { mode, icon }}
					<Button
						...
						{icon}
						variant="ghost"
						class={[view === mode ? ' ' : 'border-transparent   hover:text-white']}
						aria-label="Switch to {mode} view"
					/>
				{/each}
			</div>
			<div>
				<Button icon="icon-[ri--fullscreen-line]" variant="ghost" />
			</div>
		</div>

		{#if view === 'input'}
			<div class="flex items-center divide-x divide-white/10 overflow-x-auto border-b">
	...
	<div class="relative">
		<div class:hidden={view === 'output'}>
			<div onkeydown={handle_keydown} role="presentation">
				<Textarea
					bind:ref={textarea}
					bind:value
					oninput={on_input}
			...
			<div class="text-2 mt-1 flex justify-between px-1 text-xs">
				<span>Markdown supported</span>
			</div>
		</div>

		<div class:hidden={view === 'input'}>
			<div class="min-h-44 w-full border border-t px-3 py-2">
				{#if !value}
