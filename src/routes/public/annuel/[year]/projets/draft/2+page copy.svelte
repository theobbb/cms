<script lang="ts">
	...
	$effect(() => {
		if (project) {
			// 1. Get existing meta from project or use empty object
			const existing_meta = project.meta_files || {};

			// 2. Ensure every file in the project has at least the seed data
			const synced_meta = { ...existing_meta };

			if (project.files) {
				project.files.forEach((file: string) => {
					if (!synced_meta[file]) {
						synced_meta[file] = { ...seed_meta_file };
					}
				});
			}

			meta_files = synced_meta;
		}
	});
	...
	const onsubmit = form_action.submit(async ({ form_data }) => {
		...
		form_data.set('meta_files', JSON.stringify(meta_files));
	...
	// $effect(() => {
	// 	if (project) pop_edit_file.show({ current: project.files[3] });
	// });
</script>
