<script lang="ts">
	...
	const { collection }: { collection: CollectionModel } = $props();
	...
	async function apply_snapshot(snap: HistorySnapshot) {
		...
		for (const saved of snap) {
			...
			if (changed) {
				...
				updates.push(
					pocketbase.collection(collection.name).update(saved.id, payload, { requestKey: null })
				);
			}
		}
	...
	async function change_z_index(action: 'forward' | 'backward' | 'front' | 'back') {
		...
		item.z_index = new_z;
		close_context_menu();
		await pocketbase.collection(collection.name).update(item.id, { z_index: new_z });
	}
	...
	// ─── ROTATE STATE ────────────────────────────────────────────────────────────
	let rotating_item: RecordModel | null = $state(null);
	let rotate_center: { x: number; y: number } | null = null;
	let rotate_start_angle = 0;
	let rotate_start_value = 0;
	...
	async function move_layer(item: RecordModel, direction: -1 | 1) {
		...
		if (ti >= 0 && ti < arr.length) {
			...
			Promise.all([
				pocketbase.collection(collection.name).update(item.id, { sort_order: item.sort_order }),
				pocketbase.collection(collection.name).update(target.id, { sort_order: target.sort_order })
			]);
		}
	}
	...
	async function on_item_pointerup(e: PointerEvent, item: RecordModel) {
		if (ptr_delta < DRAG_THRESHOLD) select_item(item);
		...
		if (dragging_item) {
			...
			dragging_item = null;
			drag_rect = null;
			await pocketbase.collection(collection.name).update(it.id, { left: it.left, top: it.top });
		}
	}
	...
	async function stop_resize(e: PointerEvent) {
		if (resizing_item) {
			...
			resizing_item = null;
			await pocketbase.collection(collection.name).update(it.id, { width: it.width });
		}
	}
	...
	async function stop_margin(e: PointerEvent) {
		...
		if (margin_top_item) {
			...
			await pocketbase.collection(collection.name).update(it.id, { margin_top: it.margin_top });
		}
	...
	// ─── ROTATE ──────────────────────────────────────────────────────────────────
	function start_rotate(e: PointerEvent, item: RecordModel) {
		e.preventDefault();
		e.stopPropagation();
		(e.currentTarget as HTMLElement).setPointerCapture(e.pointerId);
		push_history();
		...
		// Compute center of the item in screen space
		const wrapper = (e.currentTarget as HTMLElement).closest(
			'.canvas-item-wrapper'
		) as HTMLElement | null;
		if (wrapper) {
			const r = wrapper.getBoundingClientRect();
			rotate_center = { x: r.left + r.width / 2, y: r.top + r.height / 2 };
		}
		rotate_start_angle =
			Math.atan2(e.clientY - (rotate_center?.y ?? 0), e.clientX - (rotate_center?.x ?? 0)) *
			(180 / Math.PI);
		rotate_start_value = item.rotate ?? 0;
	}

	function on_rotate(e: PointerEvent) {
		if (!rotating_item || !rotate_center) return;
		const angle =
			Math.atan2(e.clientY - rotate_center.y, e.clientX - rotate_center.x) * (180 / Math.PI);
		const delta = angle - rotate_start_angle;
		let new_rotate = rotate_start_value + delta;
		// Shift key → snap to 15° increments
		if (e.shiftKey) new_rotate = Math.round(new_rotate / 15) * 15;
		rotating_item.rotate = new_rotate;
	}

	async function stop_rotate(e: PointerEvent) {
		if (rotating_item) {
			(e.currentTarget as HTMLElement).releasePointerCapture(e.pointerId);
			const it = rotating_item;
			rotating_item = null;
			rotate_center = null;
			await pocketbase.collection(collection.name).update(it.id, { rotate: it.rotate });
		}
	}
	...
	async function process_layout() {
		...
		// Load all images first so we can draw in z-order (sorted_items is already sorted)
		...
		for (const { item, img } of loaded) {
			...
			// Get the bounding box with the rotation completely intact
			const r = el.getBoundingClientRect();

			// The center of a rotated bounding box perfectly aligns with the element's actual pivot center
			const cx = (r.left + r.width / 2 - cr.left) * scale;
			const cy = (r.top + r.height / 2 - cr.top) * scale;

			// Use offsetWidth/Height to get the true, unrotated physical dimensions of the element
		...
		if (!globals?.id) {
			toaster.update(toast_id, 'error', 'Globals introuvable');
			return;
		}
		const form = new FormData();
		form.append('layout', blob, 'layout.webp');
		await pocketbase.collection('globals').update(globals.id, form);
		toaster.update(toast_id, 'success', 'Layout enregistré');
	}
	...
	async function confirm_delete(post_id: string) {
		...
		await pocketbase.collection(collection.name).delete(post_id);
						...
						>
							...
							<div class="" onclick={() => select_from_sidebar(item)}>
								<div class="flex max-w-md min-w-0 flex-1 items-center gap-4">
									<Media
										src="{app.pocketbase.url}/api/files/{collection.name}/{item.id}/{item.file}"
										alt="media-{i}"
										thumbnail
									/>
								</div>
			...
			{#each sorted_items as item, i (item.id)}
				{#if item.file}
					...
					<div
						...
						class:cursor-grab={is_sel && !dragging_item && !rotating_item}
					...
					>
						<div style:aspect-ratio={item.aspect_ratio} style:width="100%" class="relative">
							{#if is_sel}
								...
								<!-- Rotation handles (one per corner, Figma-style) -->
								<!-- Top-left -->
								<button
									class="absolute -top-6 -left-6 z-50 flex size-5 cursor-crosshair items-center justify-center rounded-full bg-transparent hover:bg-blue-400/20"
									onpointerdown={(e) => start_rotate(e, item)}
									onpointermove={on_rotate}
									onpointerup={stop_rotate}
									onpointercancel={stop_rotate}
								...
								<!-- Top-right -->
								<button
									class="absolute -top-6 -right-6 z-50 flex size-5 cursor-crosshair items-center justify-center rounded-full bg-transparent hover:bg-blue-400/20"
									onpointerdown={(e) => start_rotate(e, item)}
									onpointermove={on_rotate}
									onpointerup={stop_rotate}
									onpointercancel={stop_rotate}
								...
								<!-- Bottom-left -->
								<button
									class="absolute -bottom-6 -left-6 z-50 flex size-5 cursor-crosshair items-center justify-center rounded-full bg-transparent hover:bg-blue-400/20"
									onpointerdown={(e) => start_rotate(e, item)}
									onpointermove={on_rotate}
									onpointerup={stop_rotate}
									onpointercancel={stop_rotate}
								...
								<!-- Bottom-right (shares space with resize handle; offset further) -->
								<button
									class="absolute -right-6 -bottom-6 z-50 flex size-5 cursor-crosshair items-center justify-center rounded-full bg-transparent hover:bg-blue-400/20"
									onpointerdown={(e) => start_rotate(e, item)}
									onpointermove={on_rotate}
									onpointerup={stop_rotate}
									onpointercancel={stop_rotate}
...
{#if editor.current != null && page.url.searchParams.has('editor')}
	<PostEditor items={sorted_items} {collection} />
{/if}
