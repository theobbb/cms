...
<div
	class={['relative flex flex-col items-center', absolute && 'h-full w-full']}
	bind:this={canvas.canvas_ref}
>
	{#each canvas.sorted_items as item, i (item.id)}
		{#if item.file}
			...
			<div
				class={[
					'canvas-item-wrapper border-2 transition-[border-color] duration-75',
